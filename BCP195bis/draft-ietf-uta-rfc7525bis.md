...
When a sender is approaching CL, the implementation SHOULD initiate a new handshake (or in TLS 1.3, a KeyUpdate handshake message) to rotate the session key.  When a receiver has reached IL, the implementation SHOULD close the connection.  Although these recommendations are a best practice, implementers need to be aware that it's not always easy to accomplish them in protocols that are built on top of TLS/DTLS or that re-use the TLS handshake protocol. For instance, in some protocols that depend on long-lived sessions it might not be possible to initiate a new TLS handshake or complete a TLS 1.3 KeyUpdate without tearing down the old session and setting up a new one; this operation can introduce complexities such as draining all messages off the old session while setting up the new one or even maintaining two parallel sessions at the same time. As another example, the QUIC protocol (which, as noted, uses the TLS 1.3 handshake protocol with its own record layer) defines a method for Key Update as specified in {{Section 6 of RFC9001}}, but application protocols might not be able to emulate that method given their more constrained interaction with TLS/DTLS.  As a result of these complexities, these recommendations are not mandatory.
