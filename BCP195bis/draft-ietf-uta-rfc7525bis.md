...
* TLS 1.2 does not roll the session key forward within a single session. Thus, to prevent an attack where the server's private ticket-encryption key is stolen and used to decrypt the entire content of a session (negating the concept of forward secrecy), a TLS 1.2 server SHOULD NOT resume sessions that are too old, e.g. sessions that have been open longer than two ticket-encryption key rotation periods.
